...
import RealTimeAuction from '../../components/RealTimeAuction';
      ...
      {pageLoading ? (
        ...
        <div className="w-full bg-gradient-to-r from-yellow-300/60 via-white/80 to-blue-200/60 py-8 px-6 flex flex-col items-center border-b border-yellow-100 relative">
          ...
          <div className="w-full flex flex-col sm:flex-row justify-center items-center gap-4 mt-2">
            ...
            {/* Real-Time Auction Components */}
            {auctionId && (
              <div className="w-full mt-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
                <AuctionTimer 
                  auctionId={auctionId}
                  initialTime={currentTime ? Math.max(0, Math.floor((auctionEnd - currentTime) / 1000)) : 0}
                  className="w-full"
                />
                <RealTimeAuction 
                  auctionId={auctionId}
                  timeRemaining={currentTime ? Math.max(0, Math.floor((auctionEnd - currentTime) / 1000)) : 0}
                />
              </div>
            )}
