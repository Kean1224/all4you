    ...
    <AdminAuthWrapper>
      <div className="container mx-auto py-8">
          ...
          <div className="mb-4 flex space-x-2">
            <button onClick={() => setFilter('all')} className={`px-4 py-2 rounded ${filter === 'all' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700'}`}>All</button>
            <button onClick={() => setFilter('pending')} className={`px-4 py-2 rounded ${filter === 'pending' ? 'bg-yellow-500 text-white' : 'bg-gray-200 text-gray-700'}`}>Pending</button>
            <button onClick={() => setFilter('approved')} className={`px-4 py-2 rounded ${filter === 'approved' ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-700'}`}>Approved</button>
            <button onClick={() => setFilter('blocked')} className={`px-4 py-2 rounded ${filter === 'blocked' ? 'bg-red-600 text-white' : 'bg-gray-200 text-gray-700'}`}>Blocked</button>
          </div>
                ...
                <div className="p-6">
                  ...
                  {viewingDocs ? (
                    ...
                    <div className="space-y-6">
                      ...
                      {/* Documents Available */}
                      {(selectedUser.documents?.idDocument || selectedUser.documents?.proofOfAddress) && (
                        <div>
                          ...
                          <div className="space-y-1">
                            {selectedUser.documents?.idDocument && (
                              <div className="flex items-center space-x-2 text-sm text-gray-600">
                                <span>📄</span>
                                <span>ID Document: {selectedUser.documents.idDocument}</span>
                              </div>
                            )}
                            {selectedUser.documents?.proofOfAddress && (
                              <div className="flex items-center space-x-2 text-sm text-gray-600">
                                <span>🏠</span>
                                <span>Proof of Address: {selectedUser.documents.proofOfAddress}</span>
                              </div>
                            )}
                          </div>
                        </div>
